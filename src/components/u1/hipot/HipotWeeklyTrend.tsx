/**
 * @file src/components/u1/hipot/HipotWeeklyTrend.tsx
 * @description HIPOT 주간 일별 합격률 — 면적 라인 + 포인트 강조 + 값 레이블
 *
 * 초보자 가이드:
...
import type { HipotWeeklyPoint } from "@/types/u1/hipot-analysis";

const LINE_COLORS = ["#ec4899", "#22c55e", "#f59e0b", "#ef4444", "#a855f7", "#06b6d4", "#f97316", "#3b82f6"];

interface Props {
  dailyTrend: HipotWeeklyPoint[];
}

export default function HipotWeeklyTrend({ dailyTrend }: Props) {
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
            <defs>
              {lines.map((_, i) => (
                <linearGradient key={i} id={`hipotWeekGrad${i}`} x1="0" y1="0" x2="0" y2="1">
            ...
            {lines.map((lineCode, i) => (
              <Area key={lineCode} type="monotone" dataKey={lineCode}
                ...
                fill={`url(#hipotWeekGrad${i})`} connectNulls animationDuration={800}
