/**
 * @file src/components/u1/hipot/HipotHourlyTrend.tsx
 * @description HIPOT 시간대별 검사 추이 — 그라데이션 면적 + 합격률 라인 + 요약
 *
 * 초보자 가이드:
...
import type { HipotHourlyPoint } from "@/types/u1/hipot-analysis";
...
export default function HipotHourlyTrend({ hourlyTrend }: Props) {
  ...
  return (
    <div className="flex flex-col h-full gap-1">
      ...
      <div className="flex items-center gap-4 px-3 py-1.5 rounded bg-gray-800/60 text-sm">
        ...
        <span className="text-pink-400 font-semibold">{totalInspect.toLocaleString()}건</span>
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={fullData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
            <defs>
              <linearGradient id="hipotAreaGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area yAxisId="left" type="monotone" dataKey="total" fill="url(#hipotAreaGrad)"
              stroke="#ec4899" strokeWidth={2} name="total" animationDuration={800}
              dot={{ r: 2, fill: "#ec4899", strokeWidth: 0 }}
              activeDot={{ r: 5, fill: "#f472b6", stroke: "#ec4899", strokeWidth: 2 }} />
