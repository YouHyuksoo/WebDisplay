 ...
 * @description ATE 전일 vs 당일 합격률 비교 — 그룹 바 + 증감 표시 + 요약 통계
 *
 * 초보자 가이드:
...
export default function AteYesterdayCompare({ lineStats }: Props) {
  ...
  return (
    <div className="flex flex-col h-full gap-1">
      {/* 요약 바 */}
      <div className="flex items-center gap-4 px-2 py-1 rounded bg-gray-800/60 text-[10px]">
        <span className="text-gray-500">평균</span>
        <span className="text-gray-400">전일 {avgYesterday.toFixed(1)}%</span>
        <span className="text-blue-400 font-medium">당일 {avgToday.toFixed(1)}%</span>
        <span className={`ml-auto font-bold ${avgDiff >= 0 ? "text-green-400" : "text-red-400"}`}>
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 20, right: 10, left: -10, bottom: 5 }} barCategoryGap="25%">
            ...
            <YAxis domain={[yMin, 100]} tick={{ fill: "#9ca3af", fontSize: 11 }} tickFormatter={(v: number) => `${v}%`} axisLine={{ stroke: "#374151" }} />
            <Tooltip
            ...
            <Bar dataKey="today" name="당일" radius={[3, 3, 0, 0]}>
              {chartData.map((entry, idx) => (
                <Cell key={idx} fill={entry.today < 90 ? "#ef4444" : entry.today < 95 ? "#eab308" : "url(#barBlue)"} />
              ))}
              <LabelList dataKey="diff" position="top"
                ...
                style={{ fill: "#d1d5db", fontSize: 10, fontWeight: "bold" }} />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
