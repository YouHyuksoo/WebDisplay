/**
 * @file src/components/u1/burnin/BurninYesterdayCompare.tsx
 * @description BURNIN 전일 vs 당일 합격률 비교 — 그룹 바 + 증감 표시 + 요약 통계
 *
 * 초보자 가이드:
...
import type { BurninLineStat } from "@/types/u1/burnin-analysis";

interface Props {
  lineStats: BurninLineStat[];
}

export default function BurninYesterdayCompare({ lineStats }: Props) {
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 20, right: 10, left: -10, bottom: 5 }} barCategoryGap="25%">
            <defs>
              <linearGradient id="burninBarBlue" x1="0" y1="0" x2="0" y2="1">
            ...
            <Bar dataKey="today" name="당일" radius={[3, 3, 0, 0]}>
              {chartData.map((entry, idx) => (
                <Cell key={idx} fill={entry.today < 90 ? "#ef4444" : entry.today < 95 ? "#eab308" : "url(#burninBarBlue)"} />
              ))}
