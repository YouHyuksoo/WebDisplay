/**
 * @file src/components/u1/fw/FwMonthlyHeatmap.tsx
 * @description FW 월간 ZONE별 합격률 히트맵 — 커스텀 CSS 그리드
 *
 * 초보자 가이드:
...
import type { FwHeatmapCell } from "@/types/u1/fw-analysis";
...
export default function FwMonthlyHeatmap({ heatmapData, zones }: Props) {
  const t = useTranslations("common");
  const [tooltip, setTooltip] = useState<{ cell: FwHeatmapCell; x: number; y: number } | null>(null);
  ...
  const cellMap = new Map<string, FwHeatmapCell>();
