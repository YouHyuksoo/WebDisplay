/**
 * @file src/components/u1/fw/FwWeeklyTrend.tsx
 * @description FW 주간 일별 합격률 — 면적 라인 + 포인트 강조 + 값 레이블
 *
 * 초보자 가이드:
...
import type { FwWeeklyPoint } from "@/types/u1/fw-analysis";
...
export default function FwWeeklyTrend({ dailyTrend }: Props) {
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
            <defs>
              {lines.map((_, i) => (
                <linearGradient key={i} id={`fwWeekGrad${i}`} x1="0" y1="0" x2="0" y2="1">
            ...
            {lines.map((lineCode, i) => (
              <Area key={lineCode} type="monotone" dataKey={lineCode}
                ...
                fill={`url(#fwWeekGrad${i})`} connectNulls animationDuration={800}
