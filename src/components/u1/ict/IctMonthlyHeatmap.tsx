/**
 * @file src/components/u1/ict/IctMonthlyHeatmap.tsx
 * @description ICT 월간 ZONE별 합격률 히트맵 — 커스텀 CSS 그리드
 *
 * 초보자 가이드:
...
import type { IctHeatmapCell } from "@/types/u1/ict-analysis";
...
export default function IctMonthlyHeatmap({ heatmapData, zones }: Props) {
  const t = useTranslations("common");
  const [tooltip, setTooltip] = useState<{ cell: IctHeatmapCell; x: number; y: number } | null>(null);
  ...
  const cellMap = new Map<string, IctHeatmapCell>();
