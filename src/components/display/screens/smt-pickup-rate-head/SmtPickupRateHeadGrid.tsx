/**
 * @file SmtPickupRateHeadGrid.tsx
 * @description SMT 픽업률현황(HEAD) 데이터 테이블. PB DataWindow 컬럼을 React 테이블로 변환.
...
interface SmtPickupRateHeadGridProps {
  rows: PickupRateRow[];
  isLoading: boolean;
  error: unknown;
}

export default function SmtPickupRateHeadGrid({ rows, isLoading, error }: SmtPickupRateHeadGridProps) {
  const { widths, handleMouseDown } = useGridResizer('grid-widths-smt-pickup-head', INITIAL_WIDTHS);
