/**
 * @file src/components/mxvc/repair-status/RepairDefectPie.tsx
 * @description 수리현황 불량명별 분포 도넛 차트
 *
 * 초보자 가이드:
 * - rows에서 badReasonName 기준으로 건수 집계
 * - 상위 10개 + 기타(나머지 합산) — 최대 11 슬라이스
 * - 중앙에 총 불량건수 표시
 */
'use client';

import { useMemo } from 'react';
import { useTranslations } from 'next-intl';
import ReactECharts from 'echarts-for-react';
import type { RepairStatusRow } from '@/types/ctq/repair-status';

interface Props {
  rows: RepairStatusRow[];
  height?: number;
}

const PIE_COLORS = [
  '#ef4444', '#fb923c', '#f59e0b', '#eab308', '#10b981',
  '#22d3ee', '#60a5fa', '#818cf8', '#a78bfa', '#f472b6',
  '#6b7280',
];

const MAX_SLICES = 11;

export default function RepairDefectPie({ rows, height = 240 }: Props) {
  const t = useTranslations('mxvc.repairStatus.charts');
  const uncategorized = t('uncategorized');
  const other = t('other');
  const { data, total } = useMemo(() => {
    const map = new Map<string, number>();
    for (const r of rows) {
      const key = r.badReasonName && r.badReasonName !== '-' ? r.badReasonName : (r.badReasonCode || uncategorized);
      map.set(key, (map.get(key) ?? 0) + 1);
    }
    const sorted = Array.from(map.entries())
      .map(([name, value]) => ({ name, value }))
      .sort((a, b) => b.value - a.value);

    const sliced = sorted.length <= MAX_SLICES
      ? sorted
      : [
          ...sorted.slice(0, MAX_SLICES - 1),
          { name: other, value: sorted.slice(MAX_SLICES - 1).reduce((s, r) => s + r.value, 0) },
        ];

    const totalCount = sorted.reduce((s, r) => s + r.value, 0);
    return { data: sliced, total: totalCount };
  }, [rows, uncategorized, other]);

  const hasData = total > 0;

  const option = {
    backgroundColor: 'transparent',
    tooltip: {
      trigger: 'item',
      backgroundColor: '#1f2937',
      borderColor: '#374151',
      borderWidth: 1,
      textStyle: { color: '#e5e7eb', fontSize: 12 },
      formatter: '{b}<br/>불량: <b>{c}건</b> ({d}%)',
    },
    legend: {
      type: 'scroll',
      orient: 'horizontal',
      bottom: 4,
      icon: 'circle',
      itemWidth: 8,
      itemHeight: 8,
      itemGap: 10,
      textStyle: { color: '#9ca3af', fontSize: 10, overflow: 'truncate', width: 70 },
      pageIconColor: '#9ca3af',
      pageTextStyle: { color: '#9ca3af', fontSize: 10 },
    },
    series: [{
      type: 'pie',
      radius: ['42%', '65%'],
      center: ['50%', '44%'],
      avoidLabelOverlap: true,
      itemStyle: { borderRadius: 4, borderColor: '#0f172a', borderWidth: 2 },
      label: {
        show: true,
        position: 'outside',
        formatter: '{d}%',
        fontSize: 10,
        color: '#d1d5db',
        minAngle: 10,
      },
      labelLine: { lineStyle: { color: '#4b5563' }, length: 6, length2: 4 },
      emphasis: { itemStyle: { shadowBlur: 14, shadowColor: 'rgba(0,0,0,0.5)' }, scale: true, scaleSize: 5 },
      data: data.map((d, i) => ({
        name: d.name,
        value: d.value,
        itemStyle: { color: PIE_COLORS[i % PIE_COLORS.length] },
      })),
    }],
    graphic: [{
      type: 'text',
      left: 'center',
      top: '40%',
      style: {
        text: `${total}\n건`,
        textAlign: 'center',
        fill: '#e5e7eb',
        fontSize: 15,
        fontWeight: 700,
        lineHeight: 18,
      },
    }],
    animation: true,
    animationDuration: 600,
    animationEasing: 'cubicOut' as const,
  };

  return (
    <div className="flex flex-col">
      <h3 className="text-xs font-semibold text-zinc-300 mb-1 px-3 pt-2">
        {t('defectByReason')}
        {hasData && <span className="ml-2 font-normal text-zinc-500">{t('totalCount', { count: total })}</span>}
      </h3>
      {!hasData ? (
        <div
          className="mx-3 mb-2 flex items-center justify-center border border-dashed border-zinc-700 rounded text-xs text-zinc-500"
          style={{ height }}
        >
          {t('noDefect')}
        </div>
      ) : (
        <ReactECharts option={option} style={{ height, width: '100%' }} notMerge lazyUpdate />
      )}
    </div>
  );
}
