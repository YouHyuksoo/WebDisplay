...
/** 등록 상태 배지 */
function StatusBadge({ good, defect }: { good: number; defect: number }) {
  if (good > 0 && defect > 0) {
    return (
...
export default function PostProcessSampleCard({ sampleHist }: Props) {
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm overflow-hidden shrink-0 w-[480px] flex flex-col h-full">
      {/* 헤더 */}
      ...
      {sampleHist.length === 0 ? (
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400 whitespace-nowrap">
                <th className="text-left px-4 py-2 font-medium">모델명</th>
                <th className="text-left px-3 py-2 font-medium">샘플 종류</th>
                <th className="text-center px-3 py-2 font-medium text-emerald-600 dark:text-emerald-400">양품(OK)</th>
                <th className="text-center px-3 py-2 font-medium text-red-500 dark:text-red-400">불량(NG)</th>
                <th className="text-center px-3 py-2 font-medium">합계</th>
                <th className="text-center px-3 py-2 font-medium">등록상태</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {sampleHist.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-center tabular-nums text-gray-500 dark:text-gray-400">
                    {row.totalCnt}
                  </td>
                  <td className="px-3 py-2 text-center">
                    <StatusBadge good={row.goodCnt} defect={row.defectCnt} />
                  </td>
                </tr>
              ))}
            </tbody>
