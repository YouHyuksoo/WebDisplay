/**
 * @file src/app/api/u1/burnin-analysis/daily/route.ts
 * @description BURNIN 분석 일간 API — 당일/전일 라인별 합격률, 시간대별 추이, 기계별 NG 현황
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_BURNIN_U1_DATA_RAW 테이블에서 2일치 데이터를 조회
...
export async function GET() {
  try {
    ...
    /* lineStats 조립 */
    const lineMap = new Map<string, BurninLineStat>();

    function ensureLineStat(code: string): BurninLineStat {
      if (!lineMap.has(code)) {
    ...
    const lineStats: BurninLineStat[] = [...lineMap.values()].sort((a, b) =>
      a.lineName.localeCompare(b.lineName),
    );

    /* hourlyTrend 조립 */
    const hourlyTrend: BurninHourlyPoint[] = hourlyRows.map((row) => {
      const total = Number(row.TOTAL_CNT);
      const pass = Number(row.PASS_CNT);
      return {
        hour: row.HOUR,
    ...
    /* machineNg 조립 */
    const machineNg: BurninMachineNgItem[] = machineNgRows.map((row) => ({
    ...
    const response: BurninDailyResponse = {
      lineStats,
      hourlyTrend,
      machineNg,
    ...
    console.error("BURNIN daily API error:", error);
