/**
 * @file src/app/api/u1/burnin-analysis/weekly/route.ts
 * @description BURNIN 분석 주간 API — 최근 7일 라인별 일간 합격률 추이
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_BURNIN_U1_DATA_RAW 테이블에서 최근 7일 데이터를 조회
 * 2. 날짜(DATE) x 라인(LINE_CODE) 기준으로 그룹핑
 * 3. 차트 #4(주간 추이 라인 차트) 데이터 제공
 * 4. 근무일 경계는 08:00 (TRUNC(SYSDATE-8/24))
 * 5. LAST_YN = 'Y' 조건으로 최신 검사 결과만 조회
 */

import { NextResponse } from "next/server";
import { executeQuery } from "@/lib/db";
import type { BurninWeeklyPoint, BurninWeeklyResponse } from "@/types/u1/burnin-analysis";
...
export async function GET() {
  try {
    ...
    /* dailyTrend 조립 */
    const dailyTrend: BurninWeeklyPoint[] = weeklyRows.map((row) => {
    ...
    const response: BurninWeeklyResponse = {
      dailyTrend,
    ...
    console.error("BURNIN weekly API error:", error);
