/**
 * @file src/app/api/u1/burnin-analysis/monthly/route.ts
 * @description BURNIN 분석 월간 API — 최근 30일 구역(ZONE) x 날짜 히트맵 데이터
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_BURNIN_U1_DATA_RAW 테이블에서 최근 30일 데이터를 조회
 * 2. ZONE_CODE x 날짜 기준으로 그룹핑 (ZONE_CODE가 없으면 'UNKNOWN')
 * 3. 차트 #5(월간 히트맵) 데이터 제공
 * 4. zones 배열로 Y축 레이블 목록도 함께 반환
 * 5. LAST_YN = 'Y' 조건으로 최신 검사 결과만 조회
 */

import { NextResponse } from "next/server";
import { executeQuery } from "@/lib/db";
import type { BurninHeatmapCell, BurninMonthlyResponse } from "@/types/u1/burnin-analysis";
...
async function queryMonthly(): Promise<HeatmapRow[]> {
  const sql = `
    SELECT TO_CHAR(ACTUAL_DATE, 'YYYY-MM-DD') AS WORK_DATE,
    ...
    WHERE ACTUAL_DATE >= TRUNC(SYSDATE) - 30
      AND ACTUAL_DATE <  TRUNC(SYSDATE) + 1
      AND PID IS NOT NULL
      AND NVL(SAMPLE_YN, 'N') <> 'Y'
      AND LENGTH(PID) >= 10
      AND LAST_YN = 'Y'
    GROUP BY TO_CHAR(ACTUAL_DATE, 'YYYY-MM-DD'),
...
async function queryDateRange(): Promise<DateRangeRow[]> {
  const sql = `
    SELECT TO_CHAR(TRUNC(SYSDATE)-30, 'YYYY-MM-DD') AS FROM_DATE,
...
export async function GET() {
  try {
    ...
    /* heatmapData 조립 */
    const heatmapData: BurninHeatmapCell[] = monthlyRows.map((row) => {
    ...
    const response: BurninMonthlyResponse = {
      heatmapData,
      zones,
    ...
    console.error("BURNIN monthly API error:", error);
