 ...
 * @description ATE 분석 월간 API — 최근 30일 구역(ZONE) x 날짜 히트맵 데이터
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_ATE_U1_DATA_RAW 테이블에서 최근 30일 데이터를 조회
...
export async function GET() {
  try {
    ...
    /* heatmapData 조립 */
    const heatmapData: AteHeatmapCell[] = monthlyRows.map((row) => {
    ...
    console.error("ATE monthly API error:", error);
