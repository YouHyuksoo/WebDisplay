/**
 * @file src/app/api/u1/fpy/route.ts
 * @description U1전용 직행율(First Pass Yield) API - 라인별 x 공정별 전일/당일 직행율 조회
 *
 * 초보자 가이드:
 * 1. U1 전용 5개 RAW 테이블에서 제품(PID)별 최초 검사 결과만 추출
 * 2. 직행율 = 최초검사 PASS 제품수 / 고유 제품수 x 100
 * 3. 당일 기준 90% 미만이면 A급
...
/**
 * 전일+당일 1쿼리로 공정별 직행율 조회
 *
 * 튜닝 포인트:
 * 1. 전일+당일 1쿼리 (DB 호출 최소화)
 * 2. JOIN -> EXISTS (존재 확인만 필요)
 * 3. MIN KEEP (DENSE_RANK FIRST) (Oracle 집계, 서브쿼리 1단계 제거)
 */
...
function toProcessData(row: U1FpyRow): U1FpyProcessData {
...
export async function GET(request: NextRequest) {
  try {
    ...
    /* 라인별 merge */
    const lineMap = new Map<string, U1FpyLineData>();

    function ensureLine(code: string): U1FpyLineData {
      if (!lineMap.has(code)) {
        lineMap.set(code, {
          lineCode: code,
    ...
    console.error("U1 FPY API error:", error);
    return NextResponse.json(
