/**
 * @file src/app/api/u1/hipot-analysis/daily/route.ts
 * @description HIPOT 분석 일간 API — 당일/전일 라인별 합격률, 시간대별 추이, 기계별 NG 현황
 *
 * 초보자 가이드:
...
export async function GET() {
  try {
    ...
    /* lineStats 조립 */
    const lineMap = new Map<string, HipotLineStat>();

    function ensureLineStat(code: string): HipotLineStat {
      if (!lineMap.has(code)) {
    ...
    const lineStats: HipotLineStat[] = [...lineMap.values()].sort((a, b) =>
      a.lineName.localeCompare(b.lineName),
    );

    /* hourlyTrend 조립 */
    const hourlyTrend: HipotHourlyPoint[] = hourlyRows.map((row) => {
      const total = Number(row.TOTAL_CNT);
      const pass = Number(row.PASS_CNT);
      return {
        hour: row.HOUR,
    ...
    /* machineNg 조립 */
    const machineNg: HipotMachineNgItem[] = machineNgRows.map((row) => ({
    ...
    const response: HipotDailyResponse = {
      lineStats,
      hourlyTrend,
      machineNg,
    ...
    console.error("HIPOT daily API error:", error);
