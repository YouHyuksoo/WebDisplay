/**
 * @file src/app/api/u1/hipot-analysis/weekly/route.ts
 * @description HIPOT 분석 주간 API — 최근 7일 라인별 일간 합격률 추이
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_HIPOT_U1_DATA_RAW 테이블에서 최근 7일 데이터를 조회
...
import type { HipotWeeklyPoint, HipotWeeklyResponse } from "@/types/u1/hipot-analysis";
...
export async function GET() {
  try {
    ...
    /* dailyTrend 조립 */
    const dailyTrend: HipotWeeklyPoint[] = weeklyRows.map((row) => {
    ...
    const response: HipotWeeklyResponse = {
      dailyTrend,
    ...
    console.error("HIPOT weekly API error:", error);
