/**
 * @file src/app/api/u1/fw-analysis/weekly/route.ts
 * @description FW 분석 주간 API — 최근 7일 라인별 일간 합격률 추이
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_FW_U1_DATA_RAW 테이블에서 최근 7일 데이터를 조회
...
import type { FwWeeklyPoint, FwWeeklyResponse } from "@/types/u1/fw-analysis";
...
export async function GET() {
  try {
    ...
    /* dailyTrend 조립 */
    const dailyTrend: FwWeeklyPoint[] = weeklyRows.map((row) => {
    ...
    const response: FwWeeklyResponse = {
      dailyTrend,
    ...
    console.error("FW weekly API error:", error);
