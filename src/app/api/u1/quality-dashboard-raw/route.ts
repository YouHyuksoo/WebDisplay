/**
 * @file src/app/api/u1/quality-dashboard-raw/route.ts
 * @description U1 RAW 검사 테이블 인사이트 API — 5개 U1 공정 RAW 데이터 집계
 *
 * 초보자 가이드:
 * 1. 공정별 검사량, 시간당 검사량, 라인별 생산량
 * 2. 재검사율, 라인x공정 NG, 주간 직행율 추이
 * 3. 5개 U1 RAW 테이블 UNION ALL 활용
 * 4. 기준시간 08:00 (CTQ는 10:00), LAST_YN='Y' 조건 추가
 * 5. 테이블 매핑: SERVER→U1, FT1_SMPS→FW
 */

import { NextResponse } from "next/server";
import { type NextRequest } from "next/server";
import { executeQuery } from "@/lib/db";
import { parseLines, buildLineInClause } from "@/lib/ctq/line-filter";

export const dynamic = "force-dynamic";

/** U1 전용 공정 테이블 목록 (FT→FW, SERVER→U1) */
const PROCS = [
  { key: "ICT",    table: "IQ_MACHINE_ICT_U1_DATA_RAW" },
  { key: "HIPOT",  table: "IQ_MACHINE_HIPOT_U1_DATA_RAW" },
  { key: "FW",     table: "IQ_MACHINE_FW_U1_DATA_RAW" },
  { key: "BURNIN", table: "IQ_MACHINE_BURNIN_U1_DATA_RAW" },
...
export async function GET(request: NextRequest) {
  try {
    ...
    /* 라인x공정 NG: 라인별 { lineName, ICT, HIPOT, FW, BURNIN, ATE } */
    ...
    /* 주간 직행율 추이: 날짜별 { date, ICT, HIPOT, FW, BURNIN, ATE } */
    ...
    console.error("U1 Quality Dashboard RAW API error:", error);
