/**
 * @file src/app/api/u1/ict-analysis/monthly/route.ts
 * @description ICT 분석 월간 API — 최근 30일 구역(ZONE) x 날짜 히트맵 데이터
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_ICT_U1_DATA_RAW 테이블에서 최근 30일 데이터를 조회
 * 2. ZONE_CODE x 날짜 기준으로 그룹핑 (ZONE_CODE가 없으면 'UNKNOWN')
 * 3. 차트 #5(월간 히트맵) 데이터 제공
 * 4. zones 배열로 Y축 레이블 목록도 함께 반환
 * 5. LAST_YN = 'Y' 조건으로 최종 검사 결과만 집계
 */

import { NextResponse } from "next/server";
import { executeQuery } from "@/lib/db";
import type { IctHeatmapCell, IctMonthlyResponse } from "@/types/u1/ict-analysis";
...
export async function GET() {
  try {
    ...
    /* heatmapData 조립 */
    const heatmapData: IctHeatmapCell[] = monthlyRows.map((row) => {
    ...
    const response: IctMonthlyResponse = {
      heatmapData,
      zones,
    ...
    console.error("ICT monthly API error:", error);
