/**
 * @file src/app/api/u1/ict-analysis/daily/route.ts
 * @description ICT 분석 일간 API — 당일/전일 라인별 합격률, 시간대별 추이, 기계별 NG 현황
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_ICT_U1_DATA_RAW 테이블에서 2일치 데이터를 조회
...
export async function GET() {
  try {
    ...
    /* lineStats 조립 */
    const lineMap = new Map<string, IctLineStat>();

    function ensureLineStat(code: string): IctLineStat {
      if (!lineMap.has(code)) {
    ...
    const lineStats: IctLineStat[] = [...lineMap.values()].sort((a, b) =>
      a.lineName.localeCompare(b.lineName),
    );

    /* hourlyTrend 조립 */
    const hourlyTrend: IctHourlyPoint[] = hourlyRows.map((row) => {
      const total = Number(row.TOTAL_CNT);
      const pass = Number(row.PASS_CNT);
      return {
        hour: row.HOUR,
    ...
    /* machineNg 조립 */
    const machineNg: IctMachineNgItem[] = machineNgRows.map((row) => ({
    ...
    const response: IctDailyResponse = {
      lineStats,
      hourlyTrend,
      machineNg,
      defectTypes,
    ...
    console.error("ICT daily API error:", error);
