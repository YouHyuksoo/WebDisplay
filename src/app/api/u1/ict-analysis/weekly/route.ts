/**
 * @file src/app/api/u1/ict-analysis/weekly/route.ts
 * @description ICT 분석 주간 API — 최근 7일 라인별 일간 합격률 추이
 *
 * 초보자 가이드:
 * 1. IQ_MACHINE_ICT_U1_DATA_RAW 테이블에서 최근 7일 데이터를 조회
...
import type { IctWeeklyPoint, IctWeeklyResponse } from "@/types/u1/ict-analysis";
...
export async function GET() {
  try {
    ...
    /* dailyTrend 조립 */
    const dailyTrend: IctWeeklyPoint[] = weeklyRows.map((row) => {
    ...
    const response: IctWeeklyResponse = {
      dailyTrend,
    ...
    console.error("ICT weekly API error:", error);
