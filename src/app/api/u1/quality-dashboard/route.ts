/**
 * @file src/app/api/u1/quality-dashboard/route.ts
 * @description U1 품질 분석 대시보드 API — IP_PRODUCT_WORK_QC 다차원 집계
 *
 * 초보자 가이드:
 * 1. 10종 집계: 공정별/불량코드/라인별/모델별/시간대별/불량부품/위치별/수리공정별/입고구분별/수리완료율
 * 2. 기간: 당일 08:00 ~ 익일 08:00 (CTQ 10:00과 달리 U1은 08:00 기준)
 * 3. FPY 조회: U1 전용 RAW 테이블 사용 + LAST_YN = 'Y' 필터 추가
 * 4. 테이블 매핑: SERVER → U1, FT1_SMPS → FW
 */
...
/** U1 전용 FPY 공정 테이블 매핑 (SERVER→U1, FT→FW) */
const FPY_PROCESSES = [
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("U1 Quality Dashboard API error:", error);
