/**
 * @file src/app/(ctq)/ctq/non-consecutive/page.tsx
 ...
 * 4. **라인 선택**: display-lines-ctq-non-consec localStorage 키 사용
 */
...
export default function NonConsecutivePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/ctq/non-consecutive?lines=${encodeURIComponent(selectedLines)}`);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      {/* ── WebDisplay 공통 헤더 ── */}
      <DisplayHeader title={t("pages.nonConsecutive.title")} screenId={SCREEN_ID} />
        ...
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center gap-4">
            {data && data.lines.length > 0 && (
              <>
                <SummaryBadge label={t("pages.nonConsecutive.gradeBLabel")} count={bCount} color="bg-yellow-600" />
                <SummaryBadge label={t("pages.nonConsecutive.okLabel")} count={okCount} color="bg-green-700" />
              </>
            )}
          </div>
        </div>
