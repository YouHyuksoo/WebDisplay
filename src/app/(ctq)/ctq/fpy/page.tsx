/**
 * @file src/app/(ctq)/ctq/fpy/page.tsx
 * @description CTQ 직행율(First Pass Yield) 모니터링 페이지
 *
 * 초보자 가이드:
 * 1. **대상 공정**: ICT, Hi-Pot, FT, Burn-In, ATE
 * 2. **판정 기준**: 당일 직행율 90% 미만 -> A급 (Line Stop)
 * 3. **DisplayHeader**: WebDisplay 공통 헤더 사용 (라인선택, 타이밍 설정 통합)
 * 4. **라인 선택**: display-lines-ctq-fpy localStorage 키 사용
 */
...
export default function FpyPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      {/* -- WebDisplay 공통 헤더 -- */}
      <DisplayHeader title={t("pages.fpy.title")} screenId={SCREEN_ID} />
        ...
        <div className="flex items-center justify-between max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center gap-4">
            {data && data.lines.length > 0 && aCount > 0 && (
              <SummaryBadge label={t("pages.fpy.gradeALabel")} count={aCount} color="bg-red-600" />
            )}
          </div>
        </div>
        {totalPages > 1 && (
