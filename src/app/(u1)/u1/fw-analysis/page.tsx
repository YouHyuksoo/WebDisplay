/**
 * @file src/app/(u1)/u1/fw-analysis/page.tsx
 * @description FW 분석 대시보드 — 6개 차트 3x2 그리드 레이아웃
 *
 * 초보자 가이드:
 ...
 * 5. IQ_MACHINE_FW_U1_DATA_RAW 테이블 기반 FW 검사 결과 분석
 */
...
import FwYesterdayCompare from "@/components/u1/fw/FwYesterdayCompare";
import FwHourlyTrend from "@/components/u1/fw/FwHourlyTrend";
import FwWeeklyTrend from "@/components/u1/fw/FwWeeklyTrend";
import FwMonthlyHeatmap from "@/components/u1/fw/FwMonthlyHeatmap";
import FwMachineNg from "@/components/u1/fw/FwMachineNg";
import type { FwDailyResponse, FwWeeklyResponse, FwMonthlyResponse } from "@/types/u1/fw-analysis";

const SCREEN_ID = "u1-fw";
...
export default function FwAnalysisPage() {
  ...
  const fetchDaily = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/fw-analysis/daily");
  ...
  const fetchWeekly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/fw-analysis/weekly");
  ...
  const fetchMonthly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/fw-analysis/monthly");
  ...
  return (
    <div className="h-screen bg-gray-950 dark:bg-gray-950 text-white flex flex-col overflow-hidden">
      <DisplayHeader title="FW 분석" screenId={SCREEN_ID} />
      {error && (
        ...
        {loading && !daily ? (
          ...
          <div className="grid grid-cols-3 grid-rows-2 gap-2 h-full">
            <ChartCard title="당일 라인별 합격률" badge="당일">
              {daily ? <FwDailyPassRate lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="전일 vs 당일 합격률 비교" badge="전일/당일">
              {daily ? <FwYesterdayCompare lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="시간대별 검사수량 / 합격률" badge="당일">
              {daily ? <FwHourlyTrend hourlyTrend={daily.hourlyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="주간 일별 합격률 추이" badge="주간">
              {weekly ? <FwWeeklyTrend dailyTrend={weekly.dailyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="월간 ZONE별 히트맵" badge="월간">
              {monthly ? <FwMonthlyHeatmap heatmapData={monthly.heatmapData} zones={monthly.zones} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="머신별 NG 분포" badge="당일">
              {daily ? <FwMachineNg machineNg={daily.machineNg} /> : <ChartSkeleton />}
            </ChartCard>
          </div>
        )}
      </main>
