/**
 * @file src/app/(u1)/u1/hipot-analysis/page.tsx
 * @description HIPOT 분석 대시보드 — 6개 차트 3x2 그리드 레이아웃
 *
 * 초보자 가이드:
 * 1. 3개 API를 기간별 다른 폴링 주기로 조회 (daily:30초, weekly:5분, monthly:10분)
 * 2. 6개 차트를 3열x2행 균등 그리드에 배치
 * 3. DisplayHeader/Footer는 기존 U1 패턴 따름
...
import HipotYesterdayCompare from "@/components/u1/hipot/HipotYesterdayCompare";
import HipotHourlyTrend from "@/components/u1/hipot/HipotHourlyTrend";
import HipotWeeklyTrend from "@/components/u1/hipot/HipotWeeklyTrend";
import HipotMonthlyHeatmap from "@/components/u1/hipot/HipotMonthlyHeatmap";
import HipotMachineNg from "@/components/u1/hipot/HipotMachineNg";
import type { HipotDailyResponse, HipotWeeklyResponse, HipotMonthlyResponse } from "@/types/u1/hipot-analysis";

const SCREEN_ID = "u1-hipot";
...
function ChartCard({ title, badge, children }: { title: string; badge: string; children: React.ReactNode }) {
  const badgeColor: Record<string, string> = {
    "당일": "bg-pink-900/50 text-pink-300 border border-pink-700/30",
...
export default function HipotAnalysisPage() {
  ...
  const fetchDaily = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/hipot-analysis/daily");
  ...
  const fetchWeekly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/hipot-analysis/weekly");
  ...
  const fetchMonthly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/hipot-analysis/monthly");
  ...
  return (
    <div className="h-screen bg-gray-950 dark:bg-gray-950 text-white flex flex-col overflow-hidden">
      <DisplayHeader title="HIPOT 분석" screenId={SCREEN_ID} />
      {error && (
        ...
        {loading && !daily ? (
          ...
          <div className="grid grid-cols-3 grid-rows-2 gap-2 h-full">
            <ChartCard title="당일 라인별 합격률" badge="당일">
              {daily ? <HipotDailyPassRate lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="전일 vs 당일 합격률 비교" badge="전일/당일">
              {daily ? <HipotYesterdayCompare lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="시간대별 검사수량 / 합격률" badge="당일">
              {daily ? <HipotHourlyTrend hourlyTrend={daily.hourlyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="주간 일별 합격률 추이" badge="주간">
              {weekly ? <HipotWeeklyTrend dailyTrend={weekly.dailyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="월간 ZONE별 히트맵" badge="월간">
              {monthly ? <HipotMonthlyHeatmap heatmapData={monthly.heatmapData} zones={monthly.zones} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="머신별 NG 분포" badge="당일">
              {daily ? <HipotMachineNg machineNg={daily.machineNg} /> : <ChartSkeleton />}
            </ChartCard>
          </div>
        )}
      </main>
