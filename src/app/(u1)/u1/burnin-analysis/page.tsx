/**
 * @file src/app/(u1)/u1/burnin-analysis/page.tsx
 * @description BURNIN 분석 대시보드 — 6개 차트 3x2 그리드 레이아웃
 *
 * 초보자 가이드:
 * 1. 3개 API를 기간별 다른 폴링 주기로 조회 (daily:30초, weekly:5분, monthly:10분)
 * 2. 6개 차트를 3열x2행 균등 그리드에 배치
 * 3. DisplayHeader/Footer는 기존 U1 ATE 페이지 패턴 따름
...
import BurninDailyPassRate from "@/components/u1/burnin/BurninDailyPassRate";
import BurninYesterdayCompare from "@/components/u1/burnin/BurninYesterdayCompare";
import BurninHourlyTrend from "@/components/u1/burnin/BurninHourlyTrend";
import BurninWeeklyTrend from "@/components/u1/burnin/BurninWeeklyTrend";
import BurninMonthlyHeatmap from "@/components/u1/burnin/BurninMonthlyHeatmap";
import BurninMachineNg from "@/components/u1/burnin/BurninMachineNg";
import type { BurninDailyResponse, BurninWeeklyResponse, BurninMonthlyResponse } from "@/types/u1/burnin-analysis";

const SCREEN_ID = "u1-burnin";
...
export default function BurninAnalysisPage() {
  const [daily, setDaily] = useState<BurninDailyResponse | null>(null);
  const [weekly, setWeekly] = useState<BurninWeeklyResponse | null>(null);
  const [monthly, setMonthly] = useState<BurninMonthlyResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchDaily = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/burnin-analysis/daily");
  ...
  const fetchWeekly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/burnin-analysis/weekly");
  ...
  const fetchMonthly = useCallback(async () => {
    try {
      const res = await fetch("/api/u1/burnin-analysis/monthly");
  ...
  return (
    <div className="h-screen bg-gray-950 dark:bg-gray-950 text-white flex flex-col overflow-hidden">
      <DisplayHeader title="BURNIN 분석" screenId={SCREEN_ID} />
      {error && (
        ...
        {loading && !daily ? (
          ...
          <div className="grid grid-cols-3 grid-rows-2 gap-2 h-full">
            <ChartCard title="당일 라인별 합격률" badge="당일">
              {daily ? <BurninDailyPassRate lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="전일 vs 당일 합격률 비교" badge="전일/당일">
              {daily ? <BurninYesterdayCompare lineStats={daily.lineStats} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="시간대별 검사수량 / 합격률" badge="당일">
              {daily ? <BurninHourlyTrend hourlyTrend={daily.hourlyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="주간 일별 합격률 추이" badge="주간">
              {weekly ? <BurninWeeklyTrend dailyTrend={weekly.dailyTrend} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="월간 ZONE별 히트맵" badge="월간">
              {monthly ? <BurninMonthlyHeatmap heatmapData={monthly.heatmapData} zones={monthly.zones} /> : <ChartSkeleton />}
            </ChartCard>
            <ChartCard title="머신별 NG 분포" badge="당일">
              {daily ? <BurninMachineNg machineNg={daily.machineNg} /> : <ChartSkeleton />}
            </ChartCard>
          </div>
        )}
      </main>
