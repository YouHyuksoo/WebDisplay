...
/** 라인별 생산계획 */
export interface PostProcessPlanLine {
  lineName:  string;  // 라인명 (S01 등)
  modelName: string;  // 모델명
  runNo:     string;  // RUN_NO
  planQty:   number;  // LOT_SIZE
}

/** KPI 요약 지표 */
export interface PostProcessKpi {
  planQty:   number;              // 생산 계획 합계 — F_GET_RUN_LOT_QTY(RUN_NO) 합계
  planLines: PostProcessPlanLine[]; // 라인별 계획 상세
...
export interface PostProcessMagazineRow {
  ...
  lastModifyTime: string;   // RECEIPT_DATE → 'MM-DD HH24:MI' (표시용)
  lastModifyDate: string;   // RECEIPT_DATE ISO 문자열 (경과시간 계산용)
}
