/**
 * @file smt-pickup-rate-base.ts
 * @description SMT 픽업률현황(BASE) 화면(메뉴 34) SQL 쿼리.
 * 초보자 가이드: PowerBuilder 원본 d_display_smt_pickup_rate_lst2_base.srd에서 추출한 SQL.
 * IQ_MACHINE_INSPECT_PICKUP_RATE 테이블에서 라인별 픽업률(Takeup/Miss/Realize)을 조회한다.
 * LAG 윈도우 함수로 누적 카운트 차분을 계산하고, NPM 기계는 HEAD_SUM 그대로 사용.
 *
...
/**
 * 픽업률 메인 리스트 조회 (d_display_smt_pickup_rate_lst2_base).
 * 라인별 Takeup/Miss/Realize 합계와 NG Position 상세를 조인하여 반환.
 * M 서브쿼리: 라인별 합계 + 경고등급
 * D 서브쿼리: NG 위치 상세 (LISTAGG)
 *
 * 반환 컬럼 중 ITEM_WARNING_SIGN이 'S'인 행의 수가 곧 ngCount.
 * API route에서 앱 레벨로 계산하므로 별도 ngCount 쿼리 불필요.
 */
export function sqlSmtPickupRateBaseList(): string {
  return `
SELECT M.LINE_NAME,
       M.T_CNT,
       M.M_CNT,
...
/**
 * 픽업률 NG 건수 조회 (d_display_smt_pickup_rate_ng_count_ys_base).
 ...
 */
export function sqlSmtPickupRateBaseNgCount(): string {
  return `
