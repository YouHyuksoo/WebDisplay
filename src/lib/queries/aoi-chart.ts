/**
 * @file aoi-chart.ts
 * @description AOI 차트분석 화면(메뉴 41) SQL 쿼리 모음.
 * 초보자 가이드: IQ_MACHINE_INSPECT_DATA_AOI 테이블에서 LINE_CODE, RESULT, ACTUAL_DATE를 조회.
...
/** 라인별 검사 결과 집계 (당일) */
export function sqlAoiByLine(lineClause: string): string {
  return `
SELECT
  LINE_CODE,
...
/** 직행율(FPY) 최근 7일 추이 */
export function sqlAoiFpyTrend(lineClause: string): string {
  return `
SELECT
...
export function sqlAoiSummary(lineClause: string): string {
  return `
SELECT
...
export function sqlAoiTopLines(lineClause: string): string {
  return `
SELECT * FROM (
  SELECT
    LINE_CODE,
...
/** AOI 요약 행 타입 */
export interface AoiSummaryRow {
...
/** AOI TOP 라인 행 타입 */
export interface AoiTopLineRow {
  LINE_CODE: string;
  LINE_NAME: string;
  NG_CNT: number;
  TOTAL_CNT: number;
...
/** API 전체 응답 타입 */
export interface AoiChartApiResponse {
  byLine: AoiLineRow[];
  fpyTrend: AoiFpyRow[];
  summary: AoiSummaryRow;
  topLines: AoiTopLineRow[];
  timestamp: string;
}
